...
cross-repository proof. It deliberately separates the contract/mock lane from
the real OpenShell and live Lantern Warden lane; hosted or compromised-host
claims remain unverified. It is not a Nebius deployment claim.
...
$env:LANTERN_OPENSHELL_SANDBOX = 'lantern-m5-demo'
...
The live test above passed against the local Lantern authority fixture and the
real OpenShell gateway. The final M5 artifact records the exact grant, receipt,
outcome, sandbox, and provider provenance identifiers:
[`m5-full-trust-chain.json`](evidence/m5-full-trust-chain.json).

This closeout does not claim a live Nebius deployment, a controlled hostile
...
regression/contract evidence rather than a single live end-to-end scenario. A
compromised host, gateway, Docker Desktop, kernel, or operator is outside this
proof.
## Live Lantern Warden authority-to-effect run
