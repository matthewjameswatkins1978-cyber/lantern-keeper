...
reconciliation, provenance, and retrieval. Synthetic fixtures may be committed;
private Matthew memory may not.

The current feature-line suite uses the real core and service APIs with isolated
embedded stores:
...
The older `bench/lanternbench-v1.json` runner remains a separate HTTP smoke
fixture for the pre-epistemic recall/context endpoints. It is not acceptance
proof for the current memory model. Keep exactness, integration, and
performance evidence distinct; a fast run is not evidence of epistemic
correctness.
